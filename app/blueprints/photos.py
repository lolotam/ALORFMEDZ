...
from app.utils.upload import save_uploaded_photo, delete_photo, get_photo_info, validate_upload_request, cleanup_old_uploads
    ...
    try:
        ...
        if photo_info:
            ...
            if category == 'medicines' and entity_id:
                from app.utils.database import get_medicines, save_data
...
def list_photos(category):
    ...
    try:
        ...
        if category == 'medicines' and entity_id:
            from app.utils.database import get_medicines
            medicines = get_medicines()
            medicine = next((m for m in medicines if m['id'] == entity_id), None)
            if medicine:
